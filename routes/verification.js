...
router.post('/send', avmailSend); // has to be called at register, mail sent to email id with encrypted otp
...
transporter.sendMail(mailOptions, function(error, info){
  if (error) {
    ...
    console.log('Email sent: ' + info.response + 'token' + req.body.token);
    res.send('OTP Sent');
  }
}); 
...
router.post('/get', avmailGet); // has to be called at clicking link in email ^^ sent above
function avmailGet(req, res) {
let isValid = false;
 //const isValid = otplib.authenticator.check(req.body.token, secret);
